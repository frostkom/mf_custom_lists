...
class mf_custom_list
{
	...
	function block_render_callback($attributes)
	{
		...
		if(is_array($attributes['list_id']) && count($attributes['list_id']) > 0 || $attributes['list_id'] > 0)
		{
										...
										switch($match[1])
										{
											...
											case 'list_image_no_link':
												...
												if($image_tag != '')
												{
													...
													$out_temp .= "<div class='image ".$image_class."'>";

														if($image_url != '' && $match[1] == 'list_image')
														{
															$out_temp .= "<a href='".$image_url."'>";
														}
											...
											case 'list_link':
												...
												{
													$out_temp .= get_permalink($child_page);
												}
											...
											default:
												$out_temp .= get_post_meta($child_id, $match[1], true);
											break;
										}
			...
			if(count($arr_data) > 2)
			{
				$out_temp .= "<form".apply_filters('get_form_attr', " action='#'").">"
			...
			if($out_children != '')
			{
				$out_temp .= str_replace("[children]", $out_children, $out_parent);
			}
			...
			if($out_temp != '')
			{
				$out .= "<div".parse_block_attributes(array('class' => "widget custom_list", 'attributes' => $attributes)).">".$out_temp."</div>";
			}
		}
	...
	function column_cell($column, $post_id)
	{
		...
		switch($post->post_type)
		{
			case $this->post_type:
				...
				{
					case 'items':
						$item_amount = $wpdb->get_var($wpdb->prepare("SELECT COUNT(meta_value) FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id WHERE post_status = %s AND meta_key = %s AND meta_value = '%d'", 'publish', $this->meta_prefix.'list_id', $post_id));

						$item_amount_draft = $wpdb->get_var($wpdb->prepare("SELECT COUNT(meta_value) FROM ".$wpdb->posts." INNER JOIN ".$wpdb->postmeta." ON ".$wpdb->posts.".ID = ".$wpdb->postmeta.".post_id WHERE post_status = %s AND meta_key = %s AND meta_value = '%d'", 'draft', $this->meta_prefix.'list_id', $post_id));

						echo "<a href='".admin_url("edit.php?post_type=".$this->post_type_item."&strFilterCustomList=".$post_id)."'>".$item_amount."</a>";

						if($item_amount_draft > 0)
						{
							echo " <span class='grey'>(".$item_amount_draft.")</span>";
						}
